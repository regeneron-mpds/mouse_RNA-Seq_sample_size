...
{ if (FALSE) {
  ...
  for (this.tissue in TISSUES_IN_ORDER) {
    set2 <-   setdiff(unlist(gold_standards$dchs1[[paste0(this.tissue, ",0,Padj,0.05,2")]]), "")  # genes with FC >= 2 or FC < 0.5
    set1.5 <- setdiff(unlist(gold_standards$dchs1[[paste0(this.tissue, ",0,Padj,0.05,1.5")]]), set2) # genes with (1.5 < FC < 2) OR (1/2 < FC < 2/3)
    set1.2 <- setdiff(unlist(gold_standards$dchs1[[paste0(this.tissue, ",0,Padj,0.05,1.2")]]), c(set2, set1.5)) # genes with (1.2 < FC < 1.5) OR (2/3 < FC < 5/6)
    set1 <-   setdiff(unlist(gold_standards$dchs1[[paste0(this.tissue, ",0,Padj,0.05,1")]]), c(set2, set1.5, set1.2)) # genes with (5/6 < FC < 1.2)
  ...
  saveRDS(genes.for.effect.size.exploration, 'Dchs1_genes.for.effect.size.exploration.RDS')
  # unlist(lapply(genes.for.effect.size.exploration, length))
  # length(unique(unlist(genes.for.effect.size.exploration)))
  # unique(unlist(genes.for.effect.size.exploration))
  # sort(unique(unlist(genes.for.effect.size.exploration)))
} else {
  # manuscript was written using this list, whose 'gold standard' list differs slightly from most recent version
  genes.for.effect.size.exploration <- readRDS("Dchs1_ORIG_genes.for.effect.size.exploration.RDS")  
}}
...
genewise.effect.size.res <- data.frame(expand.grid(gene = uniq.explore.genes,
                                                   ...
                                                   FC = NA, is.signif = NA, meanX.TPM = NA,
                                                   stringsAsFactors = FALSE))
# nrow(genewise.effect.size.res)
# genewise.effect.size.res[1:81, ]
...
for (i in seq(1, nrow(genewise.effect.size.res), by = block_size)) { 
  ...
  if (i %% 1000 < 100) { message(paste(i, tissue, date())) }
  ...
  for (j in seq(i, max(this_range), by = length(uniq.explore.genes))) {
    ...
    this.tpm <- this.DEres$meanY.TPM[uniq.explore.genes, tissue]
    alt.tpm <- this.DEres$meanX.TPM[uniq.explore.genes, tissue]
    downreg <- !is.na(this.fcs) & this.fcs < 1
    this.tpm[downreg] <- alt.tpm[downreg]
    
    # figure out if significant
    ...
    genewise.effect.size.res[this_tissue_range, 'is.signif'] <- tmp.pass
    genewise.effect.size.res[this_tissue_range, 'meanX.TPM'] <- this.tpm
  }  
}
head(genewise.effect.size.res)
table(genewise.effect.size.res$is.signif, useNA = 'always')

saveRDS(genewise.effect.size.res, 'Dchs1_genewise.effect.size.res.RDS')
