...
library(dplyr)
library(tidyr)
source("extras/embiggen.R")
source("extras/pub_figure_plotting_params.R")
source("extras/openPng.R")
...
USE_ORIG_GENES_FOR_S4 <- TRUE



# read in statistics from down-sampling
ovlap.results <- readRDS('Dchs1_ovlap.results.RDS')
...
for (this_TISSUE in TISSUES_IN_ORDER) {
  for (this.TPM.CUTOFF in c(0)) { # c(0, 1, 5)) {
    for (this.PVAL.CUTOFF in c(0.05)) { #c(0.01, 0.05, 0.1, 1)) {
      ...
      outfile <- paste0('Figure2B_sensitivity.FDR_TPM', this.TPM.CUTOFF, '_', PTYPE, '_', this.PVAL.CUTOFF, '_', this_TISSUE)
      openPng(outfile, p = p1, height = 900*3, width = 1200*3, res = thisres)
      tiff(paste0("figures/", outfile, '.tiff'), height = 450*4, width = 500*4, pointsize = ps, res= thisres, compression = 'lzw'); plot(p2); dev.off()
    }
  }
...
for (this_TISSUE in TISSUES_IN_ORDER) {
  for (this.FC.CUTOFF in 1.5) { # c(1, 1.2, 1.5, 2)
    for (this.TPM.CUTOFF in c(0)) { # c(0, 1, 5)) { 
      ...
      outfile <- paste0('Figure2C_sensitivity.FDR_TPM', this.TPM.CUTOFF, '_FC', this.FC.CUTOFF, '_', PTYPE, '_', this_TISSUE)
      openPng(outfile, p = p1, height = 900*3, width = 1200*3, res = thisres)
      tiff(paste0("figures/", outfile, '.tiff'), height = 450*4, width = 500*4, pointsize = ps, res= thisres, compression = 'lzw'); plot(p2); dev.off()
    }
  }
...
pairwise.results <- readRDS('Dchs1_pairwise.results.RDS')
...
############################
effect.size.results <- readRDS("Dchs1_effect.size.results.RDS")
...
{ if (USE_ORIG_GENES_FOR_S4) {
  genewise.effect.size.res <- readRDS("Dchs1_genewise.effect.size.resORIGgenes.RDS")
  this.gene <- 'Acmsd'
  SUFFIX_4B <- '_ORIGgenes'
} else {
  genewise.effect.size.res <- readRDS("Dchs1_genewise.effect.size.res.RDS")
  this.gene <- 'Trex1'
  SUFFIX_4B <- ''
}}
this_TISSUE = 'liver'
...
p <- genewise.effect.size.res %>% 
  filter(gene == this.gene, tissue == this_TISSUE, tpm.cutoff == this.TPM.CUTOFF) %>%
  mutate(N = factor(N)) %>% 
  ...
  geom_hline(yintercept = log2(N30_DEres[['Fold.Change']][this.gene, this_TISSUE]),
             linetype = ifelse(this.gene %in% gold_standards$dchs1[[paste(this_TISSUE, this.TPM.CUTOFF, 'Padj,1', sep=",")]], "solid", "dashed")) + 
  scale_x_discrete(labels=PRETTY_LABELS)

p1 <- pub_boxplot_theme_4b(p, 'png')
p2 <- pub_boxplot_theme_4b(p, 'tiff')

outfile <- paste0("Figure4B_", this.gene, "_", this_TISSUE, "_TPM", this.TPM.CUTOFF, SUFFIX_4B)
...
MIN_NUM_SIGNIF <- 10
goldstandard.FC <- 1.5
prob_correct <- readRDS(file = paste0('Dchs1_prob_correct_GSFC', goldstandard.FC, '_minSignif', MIN_NUM_SIGNIF, '.RDS'))
...
goldstandard.FC <- 1.5
MIN_NUM_SIGNIF <- 10
prob_detect_cumul <- readRDS(paste0('Dchs1_prob_detect_cumul_GSFC', goldstandard.FC, '_minSignif', MIN_NUM_SIGNIF, '.RDS'))
...
for (this_TISSUE in TISSUES_IN_ORDER) {
  for (this.pval in c(0.05)) { # c(0.01, 0.05, 0.1, 1)) {
    ...
    p1 <- pub_boxplot_theme(p, 'png', fixedcolor='blue')
    p2 <- pub_boxplot_theme(p, 'tiff', fixedcolor='blue')
    outfile <- paste0("SuppFig3_rangeFC.TPM_", this_TISSUE, "_PVAL", this.pval, "_cohenK")
...
##############################
if (USE_ORIG_GENES_FOR_S4) {
  genes.for.effect.size.exploration <- readRDS("Dchs1_genes.for.effect.size.explorationORIGgenes.RDS")  
} else {
  genes.for.effect.size.exploration <- readRDS("Dchs1_genes.for.effect.size.exploration.RDS")
}
this.TPM.CUTOFF = 0

for (this_TISSUE in TISSUES_IN_ORDER) {
  # get fold changes from N30 experiment
  this_genes <- genes.for.effect.size.exploration[[this_TISSUE]]
  tmp.N30.fcs <- data.frame(
    gene = this_genes,
    fc = log2(N30_DEres$Fold.Change[this_genes, this_TISSUE])) %>%
  ...
  outfile <- ifelse(USE_ORIG_GENES_FOR_S4, 
                    paste0("SuppFig4_manyORIGgenes_", this_TISSUE, "_tpm", this.TPM.CUTOFF),
                    paste0("SuppFig4_manygenes_", this_TISSUE, "_tpm", this.TPM.CUTOFF))
  message(outfile)
...
pairwise_WT_comp <- readRDS(paste0('Dchs1_WTcompare.results_', PTYPE, this.PVAL.CUTOFF, '.RDS'))
...
for (this.FC in c(1.5)) { # c(1, 1.2, 1.5, 2)) {
  for (this.TPM.CUTOFF in c(0)) { # c(0, 1, 5)) {
    p <- pairwise_WT_comp_long %>% 
      ...
      facet_wrap(~tissue, scales = "free", ncol = 2, labeller = label_wrap_gen(multi_line=FALSE, width=30)) +
    ...
    openPng(paste0("SuppFig11_WTvsWT_FC", this.FC, "_TPM",  this.TPM.CUTOFF, "_", PTYPE), p = p, height = 900*2/3, width = 1850/2)
    openPng(paste0("SuppFig11_WTvsWT_FC", this.FC, "_TPM",  this.TPM.CUTOFF, "_", PTYPE, "_limy30"), p = p + ylim(c(0,30)), height = 900*2/3, width = 1850/2)
  }
}
p
